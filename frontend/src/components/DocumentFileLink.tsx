...
export function DocumentFileLink({ item, file: selectedFile, className, children }: Props) {
  ...
  if (!file) return <Link className={className} to={`/dokumenty/${item.id}`}>{children ?? 'Читать →'}</Link>

  const isPdf = file.url.split(/[?#]/)[0].toLowerCase().endsWith('.pdf')
  const handleClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
    if (!isPdf && !window.confirm(`Скачать файл «${file.title || item.title}»?`)) event.preventDefault()
  }
  ...
  return <a
    ...
    download={isPdf ? undefined : file.title || true}
    onClick={handleClick}
  >{children ?? (isPdf ? 'Открыть PDF →' : 'Скачать файл →')}</a>
}
