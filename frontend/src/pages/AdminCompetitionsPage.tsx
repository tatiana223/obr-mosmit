...
type Competition={id?:number;title:string;description:string;deadline?:string;published:boolean;cover?:string;gallery?:string[];formUrl?:string;formDescription?:string}
...
export function AdminCompetitionsPage(){
 const[items,setItems]=useState<Competition[]>([]),[editing,setEditing]=useState<Competition|null>(null),[error,setError]=useState(''),[tab,setTab]=useState<'competitions'|'applications'>('competitions')
 ...
 return <>
  ...
  {tab==='competitions'?<section className="surface table-wrap"><table><thead><tr><th>Название</th><th>Срок приёма заявок</th><th>Статус</th><th/></tr></thead><tbody>{items.map(x=><tr key={x.id}><td><div className="competition-admin-title">{x.cover&&<img src={x.cover} alt=""/>}<span><b>{x.title}</b><small>{x.description}</small></span></div></td><td>{x.deadline||'Не указан'}</td><td><span className={`badge ${x.published?'published':''}`}>{x.published?'Опубликован':'Черновик'}</span></td><td><button className="text-button" onClick={()=>setEditing(x)}>Редактировать</button></td></tr>)}</tbody></table></section>:<AdminApplicationsPage embedded/>}
  {editing&&<div className="modal-backdrop"><form className="cabinet-form modal content-editor-modal" onSubmit={save}><h2>{editing.id?'Редактировать конкурс':'Новый конкурс'}</h2><label>Название<input name="title" defaultValue={editing.title} required/></label><label>Описание<textarea name="description" defaultValue={editing.description} rows={6} required/></label><label>Приём заявок до<input name="deadline" type="date" defaultValue={editing.deadline}/></label><label>Ссылка на веб-форму<input name="formUrl" type="url" defaultValue={editing.formUrl} placeholder="https://..."/></label><label>Описание веб-формы<textarea name="formDescription" rows={3} defaultValue={editing.formDescription} placeholder="Что нужно заполнить и для кого эта форма"/></label>{editing.id?<><label>Обложка конкурса<CoverUploader endpoint={`/api/admin/media/competitions/${editing.id}`} image={editing.cover}/></label><label>Изображения конкурса<MediaGalleryUploader endpoint={`/api/admin/media/competitions/${editing.id}/gallery`} images={editing.gallery}/></label></>:<p className="editor-hint">Сохраните конкурс, затем откройте его снова и добавьте обложку и изображения.</p>}<label className="check"><input name="published" type="checkbox" defaultChecked={editing.published}/> Опубликовать</label><div className="form-actions"><button type="button" className="button secondary" onClick={()=>setEditing(null)}>Отмена</button><button className="button primary">Сохранить</button></div></form></div>}
 </>
}
