...
const json = async (url: string, options?: RequestInit) => {
  ...
  if (r.status === 401 || r.status === 403) throw new Error('Войдите как администратор')
...
export function AdminCompetitionsPage() {
  ...
  const remove = async (x: Competition) => {
    if (!x.id || !confirm(`Удалить конкурс «${x.title}» и связанные заявки?`)) return
    await json(`/api/admin/competitions/${x.id}`, { method: 'DELETE' })
  ...
  const save = async (e: FormEvent<HTMLFormElement>) => {
    ...
    await json('/api/admin/competitions', {
      method: 'POST',
      body: JSON.stringify({ ...d, id: editing?.id, published: d.published === 'on' }),
    })
    setEditing(null)
    load()
  }
    ...
    <>
      ...
      <header className="page-header">
        <div>
          ...
          <h1>Конкурсы</h1>
          <p>Кабинеты организаторов конкурсов и заявки участников.</p>
        </div>
        {tab === 'competitions' && (
          <button
            className="button primary"
            onClick={() => setEditing({ title: '', published: false })}
          >
            + Новый конкурс
          </button>
        )}
          ...
          <div className="document-search">
            <div className="document-search-field">
              ...
              <input
                ...
                placeholder="Найти конкурс по названию или сроку"
              ...
              <tbody>
                {pageItems.map((x) => (
                  <tr key={x.id}>
                    ...
                    <td>{x.deadline || 'Не указан'}</td>
                    <td>
                      <span className={`badge ${x.published ? 'published' : ''}`}>
                        {x.published ? 'Опубликован' : 'Черновик'}
                      </span>
                    </td>
                    <td className="row-actions">
                      ...
                      <button className="text-button danger" onClick={() => remove(x)}>
                        Удалить
                      </button>
                    </td>
                  </tr>
                ...
                {visibleItems.length === 0 && (
                  <tr>
                    <td colSpan={4} className="empty-search-result">
                      Конкурсы не найдены. Попробуйте изменить запрос.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            {editing.id ? (
              <>
                <label>
                  Обложка конкурса
                  <CoverUploader
                    endpoint={`/api/admin/media/competitions/${editing.id}`}
                    image={editing.cover}
                  />
                </label>
                <label>
                  ...
                  <MediaGalleryUploader
                    endpoint={`/api/admin/media/competitions/${editing.id}/gallery`}
