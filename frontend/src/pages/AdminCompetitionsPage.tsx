...
type Competition={id?:number;title:string;deadline?:string;published:boolean;cover?:string;gallery?:string[]}
...
export function AdminCompetitionsPage(){
 ...
 const visibleItems=normalizedSearch?items.filter(x=>[x.title,x.deadline].some(value=>(value||'').toLocaleLowerCase('ru').includes(normalizedSearch))):items
 const pageSize=12,pageItems=visibleItems.slice((page-1)*pageSize,page*pageSize)
 const remove=async(x:Competition)=>{if(!x.id||!confirm(`Удалить конкурс «${x.title}» и связанные заявки?`))return;await json(`/api/admin/competitions/${x.id}`,{method:'DELETE'});if(editing?.id===x.id)setEditing(null);load()}
 ...
 return <>
  ...
  <header className="page-header"><div><span className="overline">Управление</span><h1>Конкурсы</h1><p>Создавайте конкурсы и рассматривайте поступившие заявки участников.</p></div>{tab==='competitions'&&<button className="button primary" onClick={()=>setEditing({title:'',published:false})}>+ Новый конкурс</button>}</header>
