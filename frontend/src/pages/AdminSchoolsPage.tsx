...
export function AdminSchoolsPage(){
 const[schools,setSchools]=useState<School[]>([]),[search,setSearch]=useState(''),[edit,setEdit]=useState<School|null>(null),[error,setError]=useState('')
 ...
 const save=async()=>{if(!edit)return;const r=await fetch(`/api/schools/${edit.id}`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify(edit)});if(!r.ok){setError('Не удалось сохранить. Войдите как администратор.');return}setEdit(null);load()}
 return <>
  ...
  <section className="surface table-wrap"><table><thead><tr><th>Школа</th><th>Заполнено разделов</th><th/></tr></thead><tbody>{visibleSchools.map(s=><tr key={s.id}><td><b>{s.title}</b><small>{htmlToPlainText(s.summary)}</small></td><td>{s.sections.filter(x=>x.fields.length).length} из {s.sections.length}</td><td><button className="text-button" onClick={()=>openEditor(s)}>Редактировать →</button></td></tr>)}{visibleSchools.length===0&&<tr><td colSpan={3} className="empty-search-result">Школы не найдены. Попробуйте изменить запрос.</td></tr>}</tbody></table></section>
