...
function prepareContent(html: string, cover?: string) {
  ...
  const addImage = (src: string, alt = '') => {
    if (src && !seen.has(src)) { seen.add(src); images.push({ src, alt }) }
  }
  document.body.querySelectorAll('img').forEach(image => {
    addImage(image.getAttribute('src') ?? '', image.getAttribute('alt') ?? '')
    const parent = image.parentElement
    image.remove()
    if (parent && !parent.textContent?.trim() && !parent.querySelector('img,video,iframe')) parent.remove()
  })
...
export function PublicNewsDetailPage() {
  ...
  useEffect(() => {
    ...
    const close = (event: KeyboardEvent) => { if (event.key === 'Escape') setViewerIndex(null) }
    const previousOverflow = document.body.style.overflow
    document.body.style.overflow = 'hidden'
    window.addEventListener('keydown', close)
    return () => { document.body.style.overflow = previousOverflow; window.removeEventListener('keydown', close) }
  }, [viewerIndex])
  if (isLoading) return <main className="article-page"><p>Загружаем новость…</p></main>
  if (isError || !item) return <main className="article-page"><Link className="article-back" to="/novosti">← Все новости</Link><h1>Новость не найдена</h1></main>
  const prepared = prepareContent(item.content ?? '', item.image)
  const allImages = [...(item.image ? [{ src: item.image, alt: item.title }] : []), ...prepared.images]
  const hideBrokenImage = (event: React.SyntheticEvent<HTMLElement>) => {
    if (event.target instanceof HTMLImageElement) {
      ...
      if (galleryLink instanceof HTMLElement) galleryLink.style.display = 'none'
  ...
  return <main className="article-page news-detail-page" onError={hideBrokenImage}>
    ...
    {prepared.images.length > 0 && <section className="news-gallery-section"><div className="news-gallery-heading"><h2>Фотоматериалы</h2></div><div className="news-gallery">
      {prepared.images.slice(0, 4).map((image, index) => <button type="button" onClick={() => setViewerIndex(index + (item.image ? 1 : 0))} key={`${image.src}-${index}`}><img src={image.src} alt={image.alt} loading="lazy" />{index === 3 && prepared.images.length > 4 && <span>+{prepared.images.length - 4}</span>}</button>)}
    </div></section>}
