...
export function NewsEditorPage() {
  ...
  useEffect(()=>{if(!id)return;loadAdminNewsItem(id).then(item=>{setTitle(item.title);setSummary(htmlToPlainText(item.summary||''));setContent(htmlToPlainText(item.content||''));setSlug(item.slug||'');setStatus(item.status);setImage(item.image);setGallery(item.gallery||[])}).catch(e=>setError(e.message))},[id])
