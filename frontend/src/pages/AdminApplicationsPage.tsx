...
export function AdminApplicationsPage({embedded=false}:{embedded?:boolean}){
 ...
 const remove=async(a:Application)=>{if(!confirm(`Удалить заявку «${a.participantName}»?`))return;await fetch(`/api/admin/applications/${a.id}`,{method:'DELETE',credentials:'include'});if(selected?.id===a.id)setSelected(null);await load()}
