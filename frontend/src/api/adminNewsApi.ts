export type AdminNewsItem={id:number;title:string;slug:string;summary:string;content:string;image?:string;gallery?:string[];status:'PUBLISHED'|'DRAFT';date:string;updatedAt:string}
const check=async(r:Response)=>{if(r.status===401||r.status===403)throw new Error('Необходимо войти как администратор');if(!r.ok)throw new Error(await r.text()||'Не удалось выполнить действие');return r}
