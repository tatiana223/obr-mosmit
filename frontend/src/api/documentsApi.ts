...
export type DocumentItem={id:string;title:string;category:string;summary:string;attachments:{title:string;url:string}[];sourceUrl:string;sectionId?:number}
export type DocumentSection={id:number;title:string;slug:string;parentId?:number;sortOrder:number}
...
export function useDocumentSections(){return useQuery<DocumentSection[]>({queryKey:['document-sections'],queryFn:async()=>{const r=await fetch('/api/document-sections');if(!r.ok)throw new Error('Не удалось загрузить разделы');return r.json()},retry:false})}
