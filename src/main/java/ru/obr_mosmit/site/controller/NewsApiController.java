...
public class NewsApiController {
    ...
    public NewsApiController(NewsRepository repository) {
        this.repository = repository;
    }

    @GetMapping
    List<NewsDto> all() {
        return repository.findByStatusOrderByPublishedAtDesc(NewsStatus.PUBLISHED)
                .stream()
                .map(this::dto)
                .toList();
    }
    ...
    private NewsDto dto(News item) {
        String date = item.getPublishedAt() == null
                ? ""
                : DATE.format(item.getPublishedAt().atZone(ZoneId.of("Europe/Moscow")));
        String publishedAt = item.getPublishedAt() == null ? "" : item.getPublishedAt().toString();
        List<String> gallery = item.getGalleryUrls() == null || item.getGalleryUrls().isBlank()
                ? List.of()
                : List.of(item.getGalleryUrls().split("\\n"));
